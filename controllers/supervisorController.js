...
const fetchTotalEvaluation = async (req) => {
    try {
        const { branchId, userId, date } = req.query;
        ...
        if (userIds.length === 0) {
            return { evalutionData: { approved: 0, rejected: 0, pending: 0, totalApprovedPages: 0, totalRejectedPages: 0, totalPendingPages: 0 } };
        }
...
const fetchDailyWisePageDoc = async (req) => {
    try {
        ...
        if (userIds.length === 0) {
            return { dailySupervisorData: { approved: 0, rejected: 0, pending: 0, totalApprovedPages: 0, totalRejectedPages: 0, totalPendingPages: 0 } };
        }
...
const fetchTeamData = async (req) => {
    try {
        ...
        const activeUserIdsSet = new Set(activities.map(activity => activity.activity_created_by_id));
...
export const fetchUsersForBranch = async (req,res,next) => {
    try {
        ...
        if (users.length === 0) {
            return { error: 'No users found under this supervisor for the specified branch' };
        }
